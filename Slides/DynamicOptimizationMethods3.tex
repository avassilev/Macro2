...
\title[]{Методи за динамично оптимизиране за задачи със стохастичен елемент. Задачи за оптимално разпределение}
...
\textbf{Задача \arabic{problemnum}:} Решете Пример I, ако фазовото уравнение е записано според конвенцията $A_{t}=(1+r)A_{t-1} + Y_t - C_t$.
\end{frame}
...
\textbf{Задача \arabic{problemnum}:} Как изглеждат горните необходими условия, ако функцията на полезност има вида $ U(C_t,N_t)=\frac{C_t^{1-\sigma}}{1-\sigma}-\frac{N_t^{1+\varphi}}{1+\varphi},~\sigma>0,~\varphi \geq 1 $?
\end{frame}



\begin{frame}\stepcounter{problemnum}
\frametitle{Задача \arabic{problemnum}: решения за спестяване и миграция}
\framesubtitle{}
...
Изведете необходимите условия за оптималност за тази задача.
\end{frame}
